...
alter table public.jam_requests
...
create policy "jam_requests_insert_own_pending"
  on public.jam_requests
  for insert
  to authenticated
  with check (
...
begin
  if to_regclass('public.creator_likes') is not null then
    execute $backfill$
      insert into public.jam_requests (requester_id, recipient_id, created_at, connected_at)
      select
        sent_like.liker_id,
        sent_like.liked_id,
        sent_like.created_at,
        case
          when received_like.liker_id is not null then greatest(sent_like.created_at, received_like.created_at)
          else null
        end as connected_at
      from public.creator_likes sent_like
      left join public.creator_likes received_like
        on received_like.liker_id = sent_like.liked_id
       and received_like.liked_id = sent_like.liker_id
      where sent_like.liker_id <> sent_like.liked_id
      on conflict (requester_id, recipient_id) do update
      set
        created_at = least(jam_requests.created_at, excluded.created_at),
        connected_at = coalesce(jam_requests.connected_at, excluded.connected_at)
    $backfill$;
  end if;
end;
$$;
