-- Per-video seen tracking for discover feed.
-- Soft TTL: rows older than 30 days count as unseen again (no hard reset).
-- Feed phases: unseen (exclude recent seen) → replay (only recent seen).
...
create index if not exists seen_videos_user_seen_at_idx
  on public.seen_videos (user_id, seen_at desc);

create index if not exists seen_videos_video_id_idx
  on public.seen_videos (video_id);

alter table public.seen_videos enable row level security;

drop policy if exists "seen_videos_select_own" on public.seen_videos;
drop policy if exists "seen_videos_insert_own" on public.seen_videos;
drop policy if exists "seen_videos_update_own" on public.seen_videos;
drop policy if exists "seen_videos_delete_own" on public.seen_videos;

create policy "seen_videos_select_own"
  on public.seen_videos
  for select
  to authenticated
  using (auth.uid() = user_id);

create policy "seen_videos_insert_own"
  on public.seen_videos
  for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy "seen_videos_update_own"
  on public.seen_videos
  for update
  to authenticated
  using (auth.uid() = user_id)
...
create policy "seen_videos_delete_own"
  on public.seen_videos
  for delete
  to authenticated
  using (auth.uid() = user_id);

create or replace function public.video_is_recently_seen(
  viewer uuid,
  target_video uuid,
...
as $$
  select exists (
    select 1
    from public.seen_videos as s
    where s.user_id = viewer
      and s.video_id = target_video
      and s.seen_at >= (now() - make_interval(days => greatest(coalesce(ttl_days, 30), 1)))
  );
$$;

revoke all on function public.video_is_recently_seen(uuid, uuid, integer) from public;
grant execute on function public.video_is_recently_seen(uuid, uuid, integer) to authenticated;
